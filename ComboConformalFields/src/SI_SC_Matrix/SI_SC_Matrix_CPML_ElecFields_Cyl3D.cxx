...
SI_SC_Matrix_CPML_ElecFields_Cyl3D::
SI_SC_Matrix_CPML_ElecFields_Cyl3D()
...
SI_SC_Matrix_CPML_ElecFields_Cyl3D::
SI_SC_Matrix_CPML_ElecFields_Cyl3D(const FieldsDefineCntr* theCntr)
    :SI_SC_Matrix_ElecFields_Cyl3D(theCntr, INCLUDING)
{
...
{
    return SI_SC_Matrix_ElecFields_Cyl3D::IsPhysDataMemoryLocated();
}
    ...
    Setup_PML_a_b()
{
    ...
    for(Standard_Integer i = 0; i < theEdgeDatas.size(); ++i){
        for(Standard_Integer dir = 0; dir < 2; ++dir){
            Matrix_Compute_a_b_SI_SC(theEdgeDatas[i], dir, dt, a, b);
    ...
    for(Standard_Integer i = 0; i < theSweptEdgeDatas.size(); ++i){
        for(Standard_Integer dir = 0; dir < 2; ++dir){
            Matrix_Compute_a_b_SI_SC(theSweptEdgeDatas[i], dir, dt, a, b);
    ...
    for(Standard_Integer i = 0; i < theEdgeDatas_Axis.size(); ++i){
        for(Standard_Integer dir = 0; dir < 2; ++dir){
            Matrix_Compute_a_b_SI_SC(theEdgeDatas_Axis[i], dir, dt, a, b);
