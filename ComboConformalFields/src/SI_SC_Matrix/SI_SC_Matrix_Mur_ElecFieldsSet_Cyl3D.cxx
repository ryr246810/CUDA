...
Clear()
{
    for(vector<SI_SC_Matrix_Mur_ElecFields*>::iterator iter = m_MurPorts_Cyl3D.begin(); iter != m_MurPorts_Cyl3D.end(); ++iter){
        SI_SC_Matrix_Mur_ElecFields* tmpFlds = *iter;
        *iter = NULL;
        delete tmpFlds;
    }
    ...
    for(Standard_Size i = 0; i < m_DimPhi; ++i){
        ...
        for(iter = thePorts->begin(); iter != thePorts->end(); ++iter){
            ...
            if(IsOpenMurPortType(currPort.m_Type)){
                SI_SC_Matrix_Mur_ElecFields* oneNewMurPort = new SI_SC_Matrix_Mur_ElecFields(m_FldsDefCntr, currPort);
