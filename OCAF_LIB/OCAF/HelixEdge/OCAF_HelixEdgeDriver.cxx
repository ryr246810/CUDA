...
Standard_Integer OCAF_HelixEdgeDriver::Execute(Handle(TFunction_Logbook)& theLogbook) const 
{
  ...
  Standard_Integer np = (Standard_Integer)anInterface.GetPeriodNum();
  ...
  Standard_Integer Num_Points_Total = Num_Period_Points+1;

  BRepBuilderAPI_MakeWire mkWire;

  for(Standard_Integer n=1; n<=np; n++){
    ...
    orgPnt[dir0] += (Standard_Real(n-1))*L;
  ...
  mkWire.Build();
  if (!mkWire.IsDone()) return HELIX_NOT_DONE;
