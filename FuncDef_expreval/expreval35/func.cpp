...
namespace
{
    ...
    // bessel_i value
    //--------------------------------------------------------------------------
    class bessel_i_FunctionNode : public FunctionNode
    {
    public:
        bessel_i_FunctionNode(Expression *expr) : FunctionNode(expr)
        {
        ...
        {
            double result = 0;
            //   boost::math::cyl_bessel_i(m_nodes[0]->Evaluate(), m_nodes[1]->Evaluate());
            //   result = std::cyl_bessel_i(m_nodes[0]->Evaluate(), m_nodes[1]->Evaluate());
            if (m_nodes[0]->Evaluate() == 0.0)
            {
                result = alglib::besseli0(m_nodes[1]->Evaluate());
            }
            if (m_nodes[0]->Evaluate() == 1.0)
            {
                result = alglib::besseli1(m_nodes[1]->Evaluate());
            }
            return result;
        }
    };

    class bessel_i_FunctionFactory : public FunctionFactory
    {
    public:
        ...
        {
            return new bessel_i_FunctionNode(expr);
        }
    };

    // bessel_j value
    //--------------------------------------------------------------------------
    class bessel_j_FunctionNode : public FunctionNode
    {
    public:
        bessel_j_FunctionNode(Expression *expr) : FunctionNode(expr)
        {
            SetArgumentCount(2, 2, 0, 0);
        }
        ...
        {
            double result = 0;
            //boost::math::cyl_bessel_j(m_nodes[0]->Evaluate(), m_nodes[1]->Evaluate());
            //double result = std::cyl_bessel_j(m_nodes[0]->Evaluate(), m_nodes[1]->Evaluate());
            if (m_nodes[0]->Evaluate() == 0.0)
            {
                result = alglib::besselj0(m_nodes[1]->Evaluate());
            }else if (m_nodes[0]->Evaluate() == 1.0)
            {
                result = alglib::besselj1(m_nodes[1]->Evaluate());
            }else{
                result = alglib::besseljn(m_nodes[0]->Evaluate(), m_nodes[1]->Evaluate());
            }
            return result;
        }
    };

    class bessel_j_FunctionFactory : public FunctionFactory
    {
    public:
        string GetName() const
        {
        ...
        FunctionNode *DoCreate(Expression *expr)
        {
            return new bessel_j_FunctionNode(expr);
        }
    };

    // bessel_k value
    //--------------------------------------------------------------------------
    class bessel_k_FunctionNode : public FunctionNode
    {
    public:
        bessel_k_FunctionNode(Expression *expr) : FunctionNode(expr)
        {
            SetArgumentCount(2, 2, 0, 0);
        }
        ...
        {
            /*
      int nInput= (int)m_nodes[0]->Evaluate();
      double vInput = m_nodes[1]->Evaluate();
      if(vInput==0) vInput+=1e-10;
      double result = alglib::besselkn(nInput, vInput);
      //*/
            ...
            if (vInput == 0)
                vInput += 1e-10;
            double result = 0;
            //boost::math::cyl_bessel_k(m_nodes[0]->Evaluate(), vInput);
            //*/

            //double result = boost::math::cyl_bessel_k(m_nodes[0]->Evaluate(), m_nodes[1]->Evaluate());
            //double result = std::cyl_bessel_k(m_nodes[0]->Evaluate(), m_nodes[1]->Evaluate());
            if (m_nodes[0]->Evaluate() == 0.0)
            {
                result = alglib::besselk0(m_nodes[1]->Evaluate());
            }else if (m_nodes[0]->Evaluate() == 1.0)
            {
                result = alglib::besselk1(m_nodes[1]->Evaluate());
            }else{
                result = alglib::besselkn(m_nodes[0]->Evaluate(), m_nodes[1]->Evaluate());
            }
            return result;
        }
    };

    class bessel_k_FunctionFactory : public FunctionFactory
    {
    public:
        string GetName() const
        {
        ...
        FunctionNode *DoCreate(Expression *expr)
        {
            return new bessel_k_FunctionNode(expr);
        }
    };
    //*/
    ...
    // STEP value
    //--------------------------------------------------------------------------
    class STEP_FunctionNode : public FunctionNode
    {
    public:
        STEP_FunctionNode(Expression *expr) : FunctionNode(expr)
        {
            SetArgumentCount(2, 2, 0, 0);
        }
        ...
        {
            double result = 0;
            double tmp1 = m_nodes[0]->Evaluate();
            double tmp2 = m_nodes[1]->Evaluate();
            if (tmp1 > tmp2)
                result = 1;
            else if (tmp1 == tmp2)
                result = 0.5;
    ...
    class STEP_FunctionFactory : public FunctionFactory
    {
    public:
        string GetName() const
        {
        ...
        FunctionNode *DoCreate(Expression *expr)
        {
            return new STEP_FunctionNode(expr);
        }
    };

    // THETA value
    //--------------------------------------------------------------------------
    class THETA_FunctionNode : public FunctionNode
    {
    public:
        THETA_FunctionNode(Expression *expr) : FunctionNode(expr)
        {
    ...
    class THETA_FunctionFactory : public FunctionFactory
    {
    public:
        string GetName() const
        {
        ...
        FunctionNode *DoCreate(Expression *expr)
        {
            return new THETA_FunctionNode(expr);
        }
    };

    // RAMP value
    //--------------------------------------------------------------------------
    class RAMP_FunctionNode : public FunctionNode
    {
    public:
        RAMP_FunctionNode(Expression *expr) : FunctionNode(expr)
        {
            SetArgumentCount(1, 1, 0, 0);
        }
        ...
        double DoEvaluate()
        {
            double tmpData = m_nodes[0]->Evaluate();
#ifdef _WIN32
            double result = max(0.0, min(1.0, tmpData));
#else
            double result = fmax(0, fmin(1.0, tmpData));
#endif
            return result;
        }
    };

    class RAMP_FunctionFactory : public FunctionFactory
    {
    public:
        string GetName() const
        {
        ...
        FunctionNode *DoCreate(Expression *expr)
        {
            return new RAMP_FunctionNode(expr);
        }
    };

    // SMOOTH_RAMP value
    //--------------------------------------------------------------------------
    class SMOOTH_RAMP_FunctionNode : public FunctionNode
    {
    public:
        SMOOTH_RAMP_FunctionNode(Expression *expr) : FunctionNode(expr)
        {
            SetArgumentCount(1, 1, 0, 0);
        }
        ...
        double DoEvaluate()
        {
            double inputData = m_nodes[0]->Evaluate();
#ifdef _WIN32
            double tmpData = sin(0.5 * M_PI * max(0.0, min(1.0, inputData)));
#else
            double tmpData = sin(0.5 * M_PI * fmax(0, fmin(1.0, inputData)));
#endif
            double result = tmpData * tmpData;
            return result;
        }
    };

    class SMOOTH_RAMP_FunctionFactory : public FunctionFactory
    {
    public:
        string GetName() const
        {
        ...
        FunctionNode *DoCreate(Expression *expr)
        {
            return new SMOOTH_RAMP_FunctionNode(expr);
        }
    };
...
#define ADDFUNCTION(name)                                                  \
    ...
    ADDFUNCTION(bessel_i);
    ADDFUNCTION(bessel_j);
    ADDFUNCTION(bessel_k);

    ADDFUNCTION(STEP);
    ADDFUNCTION(THETA);
    ADDFUNCTION(RAMP);
    ADDFUNCTION(SMOOTH_RAMP);
