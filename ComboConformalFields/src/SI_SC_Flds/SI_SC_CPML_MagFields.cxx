...
SI_SC_CPML_MagFields(const FieldsDefineCntr* theCntr)
  :SI_SC_MagFields(theCntr, INCLUDING)
{
}
...
Setup()
{
  ...
  m_FaceMagFlds->clearMaterials();
  m_FaceMagFlds->AppendingMaterial(PML);
  m_FaceMagFlds->SetDelTime(dt);
  m_FaceMagFlds->Setup();


  m_SweptFaceMagFlds = new SweptFaceMagFldsBase(GetFldsDefCntr(), m_Rule);
  m_SweptFaceMagFlds->clearMaterials();
  m_SweptFaceMagFlds->AppendingMaterial(PML);
...
{
  return  SI_SC_MagFields::IsPhysDataMemoryLocated();
}
